...
const FluencyFeedback: React.FC<FluencyFeedbackProps> = ({ data }) => {
  // Logic to separate evaluation by strengths/weaknesses (simplified for mock/simulated data)
  const strengths = data.evaluation.filter((e, i) => i % 2 === 0);
  const weaknesses = data.evaluation.filter((e, i) => i % 2 !== 0);
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={[styles.card, { backgroundColor: "#F0FFF5" }]}>
        ...
        {strengths.map((item, idx) => (
          <Text key={idx} style={styles.cardContent}>
            {item.description}
          </Text>
        ))}
      </View>
      ...
      <View style={[styles.card, { backgroundColor: "#EFF6FF" }]}>
        ...
        <Text style={styles.cardContent}>
          To improve, work on using more varied sentence structures and smoother transitions between ideas.
        </Text>
      </View>
...
const styles = StyleSheet.create({
  ...
  correctionSubBlockRed: {
    backgroundColor: "#FFF8F8",
    padding: 16,
    borderBottomWidth: 1,
    borderBottomColor: "#FEE2E2",
  },
  correctionSubBlockGreen: {
